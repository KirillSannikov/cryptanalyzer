...
public class BruteForceForDefaultText {
    private static int modulo(int n, int modulus) {
        if (n < 0) return n % modulus + modulus;
        return n % modulus;
    }
    public static void defaultTextBruteForce() {

        ConstBruteForceDefault constBruteForceDefault = new ConstBruteForceDefault();
        String text = "";
        String text1 = "";
        ...
        try (BufferedReader bufferedWriter = new BufferedReader(new FileReader("cipherTextDefault.txt"))) {
            ...
            System.out.println(constBruteForceDefault.exceptionFile + e.getMessage());
        }
        ...
        while (key!=33){
            ...
            if (b != 0) {
                try{
                    ...
                    fileWriter.close();
                    System.out.println(constBruteForceDefault.ready);
                    System.out.println(constBruteForceDefault.decryptionText + key);
                    System.out.println(constBruteForceDefault.fileLocation);
                    break;
                } catch(Exception e){
                    System.out.println(constBruteForceDefault.ciphertextGenerationError);
                }
            }
            key++;
        }
